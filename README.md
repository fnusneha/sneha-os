...
**Android:** `./android` — Kotlin shell that wraps the same URL and adds
home-screen widget + 10 PM notification.
...
```bash
cd android
export JAVA_HOME=/Users/you/Library/Java/JavaVirtualMachines/corretto-17.0.12/Contents/Home
./gradlew :app:assembleDebug
adb install -r app/build/outputs/apk/debug/app-debug.apk
```

The Android app requires only one env-set thing: the dashboard URL,
baked in at build time (defaults to the prod Render URL; override with
`-PbaseUrl=https://…`).
...
| Android app   | Debug APK built locally, sideloaded             | `gradlew assemble`  |
